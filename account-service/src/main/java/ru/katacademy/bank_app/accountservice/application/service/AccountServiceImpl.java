...
import ru.katacademy.bank_app.accountservice.exception.AccountNotFoundExceptionResolver;
...
public class AccountServiceImpl implements AccountService {
    ...
    /**
     * Создаёт аккаунт для пользователя userId. Проверяет, что пользователь существует,
     * и что у него не больше 5 аккаунтов.
     */
    @Override
    public AccountEntity createAccount(UserEntity user, AccountNumber accountNumber, Money initialBalance) {
        ...
        long count = accountJpaRepository.countByUserId(user.getId());
        if (count >= 5) {
            throw new MaxAccountsExceededException("User " + user + " уже имеет " + count + " аккаунтов");
        }

        // создание аккаунта
        AccountEntity account = new AccountEntity(accountNumber, user, initialBalance, AccountStatus.ACTIVE, LocalDateTime.now());
